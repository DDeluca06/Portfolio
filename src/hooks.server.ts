...
const RATE_LIMIT_READS_PER_MINUTE = parseInt(process.env.RATE_LIMIT_READS_PER_MINUTE || '100', 10);
const RATE_LIMIT_WRITES_PER_MINUTE = parseInt(process.env.RATE_LIMIT_WRITES_PER_MINUTE || '20', 10);
...
// Rate limiting check
function checkRateLimit(clientIP: string, isRead: boolean): { allowed: boolean; retryAfter?: number } {
  const now = Date.now();
  const limit = isRead ? RATE_LIMIT_READS_PER_MINUTE : RATE_LIMIT_WRITES_PER_MINUTE;
...
export const handle: Handle = async ({ event, resolve }) => {
      ...
      return new Response(
        ...
        }),
        {
          ...
          headers: {
            ...
            'X-RateLimit-Limit': String(isRead ? RATE_LIMIT_READS_PER_MINUTE : RATE_LIMIT_WRITES_PER_MINUTE),
  ...
  securityHeaders.set('Permissions-Policy', 'accelerometer=(), camera=(), geolocation=(), gyroscope=(), magnetometer=(), microphone=(), payment=(), usb=()');
  
  // HSTS header for HTTPS
  if (ENABLE_HSTS) {
    securityHeaders.set('Strict-Transport-Security', `max-age=${HSTS_MAX_AGE}; includeSubDomains`);
  }
